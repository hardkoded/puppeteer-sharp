...
// NOTE: This intentionally uses dynamic function construction to turn a string
// representation of a function into an actual Function. The input originates
// from controlled library code and this is necessary to support injected
// functions. Keep this usage under review for security.
const createFunction = (functionValue) => {
    let fn = createdFunctions.get(functionValue);
    if (fn) {
        return fn;
    }
    // NOSONAR: dynamic function construction is intentional here to evaluate
    // function source strings produced by the library at build time.
    fn = new Function(`return ${functionValue}`)(); // NOSONAR
    createdFunctions.set(functionValue, fn);
    return fn;
};

// src/injected/ARIAQuerySelector.ts
const ARIAQuerySelector_exports = {};
...
const CustomQuerySelector_exports = {};
    ...
    register(name, handler) {
        if (!handler.queryOne && handler.queryAll) {
            ...
            handler.queryOne = (node, selector) => {
                // Get first result from possibly-iterable querySelectorAll result
                const iterator = querySelectorAll(node, selector)[Symbol.iterator]();
...
const PierceQuerySelector_exports = {};
...
const TRIVIAL_VALUE_INPUT_TYPES = /* @__PURE__ */ new Set([
    "checkbox",
    "image",
    "radio",
]);
...
const UNSUITABLE_NODE_NAMES = /* @__PURE__ */ new Set(["SCRIPT", "STYLE"]);
...
const observedNodes = /* @__PURE__ */ new WeakSet();
...
const TextQuerySelector_exports = {};
...
const XPathQuerySelector_exports = {};
...
const injected_default = PuppeteerUtil;
